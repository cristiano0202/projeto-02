(function () {
  ...
  function getTip(equipmentName = '', mode = 'equipment') {
    if (mode === 'bill') {
      return 'Compare os maiores equipamentos da casa com o consumo total da conta. Chuveiro, ar-condicionado, geladeira e cooktop costumam ter maior impacto.';
    }

    const name = equipmentName.toLowerCase();

    if (name.includes('chuveiro')) return 'Reduza 5 minutos no banho e use a posição verão sempre que possível.';
    if (name.includes('ar-condicionado') || name.includes('ar condicionado')) return 'Mantenha em 23°C ou 24°C, limpe os filtros e feche portas e janelas.';
    if (name.includes('geladeira')) return 'Evite abrir a porta muitas vezes e confira a borracha de vedação.';
    if (name.includes('micro')) return 'Use apenas para aquecer pequenas porções e evite descongelamentos longos.';
    if (name.includes('lavar')) return 'Use a capacidade máxima recomendada e evite ciclos pequenos.';
    if (name.includes('televisor') || name.includes('tv')) return 'Ative o modo economia e desligue da tomada quando ficar muito tempo sem uso.';
    if (name.includes('cooktop') || name.includes('fogão')) return 'Use panelas adequadas e desligue alguns minutos antes do fim do preparo.';
  ...
  function calculateEquipment({ powerWatts, hours, minutes, days, tariff, totalBillKwh }) {
    ...
    if (!Number.isFinite(power) || power <= 0) throw new Error('Informe uma potência válida.');
    if (!Number.isFinite(h) || h < 0 || h > 24) throw new Error('Horas devem estar entre 0 e 24.');
    if (!Number.isFinite(min) || min < 0 || min > 59) throw new Error('Minutos devem estar entre 0 e 59.');
    if ((h + min) <= 0) throw new Error('Informe um tempo de uso maior que zero.');
    if (!Number.isFinite(d) || d <= 0 || d > 31) throw new Error('Dias devem estar entre 1 e 31.');
    if (!Number.isFinite(t) || t <= 0) throw new Error('Informe uma tarifa válida.');

    const hoursPerDay = h + (min / 60);
    const monthlyKwh = (power * hoursPerDay * d) / 1000;
    const monthlyCost = monthlyKwh * t;
    const billSharePercent = Number.isFinite(totalKwh) && totalKwh > 0
  ...
  function calculateBill({ billCost, billKwh, billTariff, billDays }) {
    ...
    if (!Number.isFinite(days) || days <= 0 || days > 31) throw new Error('Dias do período devem estar entre 1 e 31.');
    ...
    const monthlyKwh = Number.isFinite(informedKwh) && informedKwh > 0
      ? informedKwh
      : cost / tariff;

    const monthlyCost = Number.isFinite(cost) && cost > 0
      ? cost
      : monthlyKwh * tariff;
  ...
  async function loadEquipments() {
    try {
      ...
      return data.equipamentos?.length ? data.equipamentos : (data.equipments?.length ? data.equipments : FALLBACK_EQUIPMENTS);
    } catch (error) {
  ...
  function setEquipmentResult(result, equipmentName) {
    ...
    document.querySelector('[data-result-extra]').textContent = result.billSharePercent === null
  ...
  function setBillResult(result) {
    ...
    document.querySelector('[data-result-extra]').textContent = `Média de ${window.EcoApi.formatKwh(result.dailyAverageKwh)} por dia. Potência média equivalente: ${result.averageContinuousWatts} W ligados continuamente.`;
  }
  ...
  function setupCalculator() {
    ...
    const customPowerBox = document.querySelector('[data-custom-power-box]');
    ...
    function updateButtons() {
      if (currentMode === 'equipment') {
        equipmentModeButton.className = 'eco-btn-primary';
        billModeButton.className = 'eco-btn-secondary';
        saveButton.classList.remove('hidden');
      } else {
        equipmentModeButton.className = 'eco-btn-secondary';
        billModeButton.className = 'eco-btn-primary';
    ...
    function setMode(mode) {
      ...
      document.querySelector('[data-result-state]').textContent = mode === 'equipment'
    ...
    function updatePowerBySelection() {
      ...
      if (customPowerBox) {
        customPowerBox.classList.toggle('hidden', !isCustom);
      }
    ...
    function runEquipmentCalculation({ silent = false } = {}) {
      ...
      lastPayload = {
        equipmentId: equipmentSelect.value !== 'custom' && equipmentSelect.value ? Number(equipmentSelect.value) || null : null,
        equipmentName,
      ...
      if (!silent) window.EcoApi.showToast('Cálculo por equipamento realizado.', 'success');
      return result;
    }
    ...
    function runBillCalculation({ silent = false } = {}) {
      ...
      if (!silent) window.EcoApi.showToast('Cálculo da conta mensal realizado.', 'success');
      return result;
    }
    ...
    function tryAutoCalculate() {
      try {
        if (currentMode === 'equipment') {
          if (!equipmentSelect.value || !powerInput.value || !form.days.value || !form.tariff.value) return;
        }

        if (currentMode === 'bill') {
          if (!form.billCost.value && !form.billKwh.value) return;
        }

        runCalculation({ silent: true });
      } catch (error) {
        // Cálculo automático ignora erro enquanto o usuário ainda digita.
      }
    }
    ...
    ['input', 'change'].forEach((eventName) => {
      form.querySelectorAll('input').forEach((input) => {
        input.addEventListener(eventName, () => tryAutoCalculate());
      });
    });
    ...
    if (saveButton) {
      saveButton.addEventListener('click', async () => {
        try {
          if (currentMode !== 'equipment') {
            window.EcoApi.showToast('O salvamento está disponível apenas para simulação por equipamento.', 'info');
            return;
          }

          if (!lastPayload) runEquipmentCalculation({ silent: true });

          if (!window.EcoApi.isAuthenticated()) {
            window.EcoApi.showToast('Entre na sua conta para salvar a simulação.', 'info');
            setTimeout(() => {
          ...
          window.EcoApi.showToast('Simulação salva no histórico.', 'success');
        } catch (error) {
          window.EcoApi.showToast(error.message, 'error');
        } finally {
